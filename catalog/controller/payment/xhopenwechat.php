<?php
class ControllerPaymentxhopenwechat extends Controller {
	var $id='xhopenwechat';
	public function index() {
		...
		$data['payment_submit_url']            = $this->url->link('payment/xhopenwechat/send');

		if(version_compare(VERSION,'2.0.0.0', '>=')){
		    if(version_compare(VERSION,'2.2.0.0', '>=')){
		        return $this->load->view('payment/xhopenwechat', $data);
		    }
		    return $this->load->view('default/template/payment/xhopenwechat.tpl', $data);
		}else{
		    $this->data = $data;
		    $this->template = 'default/template/payment/xhopenwechat.tpl';
	...
	public function send() {
	    $this->load->model('checkout/order');
	    $this->language->load('payment/xhopenwechat');
	    ...
	    $turl = rtrim($this->config->get('xhopenwechat_transaction_url'),'/');
	    $order_id                 = $this->session->data['order_id'];
	    $succeed_status = $this->config->get('xhopenwechat_order_succeed_status_id');
	    $wait_status = $this->config->get('xhopenwechat_order_payWait_status_id');
	    ...
	    $rate             = round(floatval($this->config->get('xhopenwechat_rate')),3);
		$json = array();
		$data=array(
		    'version'   => '1.1',//api version
		    'lang'       => $this->session->data['language'],
		    'is_app'    => $this->isWebApp()?'Y':'N',
		    'plugins'   => 'opencart-wechat',
		    'appid'     => $this->config->get('xhopenwechat_account'),
		    ...
		    'description'=> null,
		    'time'      => time(),
		    'notify_url'=> $this->url->link('payment/xhopenwechat/notify'),
		    'return_url'=> $this->url->link('checkout/success', '', 'SSL'),
		    'callback_url'=>$this->url->link('checkout/order', '', 'SSL'),
		    'nonce_str' => str_shuffle(time())
		);
		
		$hashkey          = $this->config->get('xhopenwechat_secret');
		...
		try {
		    ...
		    if(!isset( $result['hash'])|| $hash!=$result['hash']){
		        throw new Exception(__('Invalid sign!',XH_xhopenwechat_Payment),40029);
		    }
	...
	public function notify(){
	    ...
	    if(isset($data['plugins'])&&$data['plugins']!='opencart-wechat'){
	        return;
	    }
	    
	    $appkey =$this->config->get('xhopenwechat_secret');
